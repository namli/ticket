...
- ticket-set 1.0.0: New plugin, `tk set <id> <field> <value> [--no-note]` changes title, priority (0-4), type, assignee, tags (`a,b` replaces, `+a,-b` edits) or parent (must exist, not the ticket itself or a descendant) without an editor; `none` removes assignee, tags or parent; refuses status, deps, links, id and created with a pointer to their commands; rewrites the front matter only, keeps `/`, `&` and `\` in values, and writes an `Edited: <field> <old> -> <new>` note
